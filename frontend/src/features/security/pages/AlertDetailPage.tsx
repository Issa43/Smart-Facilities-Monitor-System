...
export function AlertDetailPage() {
  ...
  const updateStatus = useMutation({
    mutationFn: () => setAlertStatus(alertId, 'reviewed'),
    onSuccess: (updated) => {
    ...
    onError: (error) =>
      showToast({
        tone: 'critical',
        title: 'تعذّر تحديث التنبيه',
        description: error instanceof Error ? error.message : undefined,
      }),
  })

  const dismiss = useMutation({
    mutationFn: () => dismissAlert(alertId, dismissReason),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: qk.alerts.all })
      queryClient.invalidateQueries({ queryKey: qk.stats.security })
      setDismissOpen(false)
      setDismissReason('')
      showToast({ tone: 'neutral', title: 'تم تصنيف التنبيه كإنذار كاذب' })
    },
    onError: (error) =>
      showToast({
        tone: 'critical',
        title: 'تعذّر رفض التنبيه',
        description: error instanceof Error ? error.message : undefined,
      }),
  })

  const snapshot = useMutation({
    mutationFn: () => downloadAlertSnapshot(alertId),
    onError: (error) =>
      showToast({
        tone: 'critical',
        title: 'تعذّر تنزيل اللقطة',
        description: error instanceof Error ? error.message : undefined,
      }),
  })
  ...
  const escalate = useMutation({
    ...
    onError: (error) =>
      showToast({
        tone: 'critical',
        title: 'تعذّر تحويل التنبيه إلى حادث',
        description: error instanceof Error ? error.message : undefined,
      }),
  })
  ...
  const canAct = alert.status === 'new' || alert.status === 'reviewed'
  ...
  return (
    <>
        ...
        actions={
          canAct && (
            <>
              ...
              {alert.status === 'new' && (
                <Button variant="ghost" onClick={() => updateStatus.mutate()}>
          ...
          <div className={styles.heroBody}>
            ...
            <DescriptionList
              items={[
                ...
                { label: 'الكاميرا', value: alert.cameraCode ?? '—' },
                { label: 'ROI', value: alert.roiIdentifier ?? '—' },
                {
                  label: 'الثقة',
                  value: alert.confidence == null ? '—' : `${Math.round(alert.confidence * 100)}%`,
                },
                {
                  label: 'حدث الكاميرا',
                  value: alert.cameraEventId ? (
                    <Link to={`/security/events/${alert.cameraEventId}`}>فتح الحدث المرتبط</Link>
                  ) : (
                    '—'
                  ),
                },
      ...
      <SplitGrid>
        <Panel title="لقطات نظام المراقبة" subtitle="الصور المرفقة تلقائياً من الكاميرا المصدر">
          {alert.snapshotAvailable ? (
            <StateCard
              bare
              title="لقطة محمية متاحة"
              description={`أُرفقت من ${alert.source} ${formatRelative(alert.detectedAt)}. يتطلب التنزيل صلاحية المنشأة.`}
              action={
                <Button loading={snapshot.isPending} onClick={() => snapshot.mutate()}>
            ...
            <StateCard bare title="لا توجد لقطة مرفقة بهذا التنبيه" />
          )}
        </Panel>
      ...
      <Modal
        ...
        title="تصنيف التنبيه كإنذار كاذب"
        subtitle="سبب الرفض مطلوب حسب عقد النظام الخلفي."
        footer={
          <>
