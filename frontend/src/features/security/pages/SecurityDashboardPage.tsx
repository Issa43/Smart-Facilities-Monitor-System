...
import { listSecurityFacilities as listFacilities } from '@/api/security'
          ...
          <KpiCard
            ...
            value={
              stats?.averageResponseMinutes === null
                ? 'غير متاح'
                : `${formatNumber(stats?.averageResponseMinutes ?? 0)} دقيقة`
            }
            icon="progress"
            tone="accent"
            loading={statsQuery.isPending}
      ...
      <Section>
        <div className={shared.chartRow}>
          <Panel title="اتجاه التنبيهات" subtitle="عدد التنبيهات الواردة شهرياً خلال آخر ستة أشهر">
            ...
            ) : (stats?.alertTrend.length ?? 0) === 0 ? (
              <StateCard bare title="لا توجد تنبيهات مسجلة خلال آخر ستة أشهر" />
            ) : (
              <ColumnChart
          ...
          <Panel title="التنبيهات حسب النوع">
            ...
            ) : (stats?.alertsByType.length ?? 0) === 0 ? (
              <StateCard bare title="لا توجد تنبيهات لعرض توزيع أنواعها" />
            ) : (
              <DonutChart
              ...
              {openIncidents.map((incident) => (
                ...
                >
                  <div className={shared.stripHead}>
                    <div>
                      <span className={shared.stripName}>{alertTypeLabel(incident.type)}</span>
