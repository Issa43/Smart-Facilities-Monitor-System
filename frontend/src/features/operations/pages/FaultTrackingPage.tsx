...
export function FaultTrackingPage() {
  ...
  const advance = useMutation({
    mutationFn: (input: {
      ...
      input.status === 'resolved'
        ? resolveFault(input.id, input.rootCause ?? '', input.resolution ?? '')
    ...
    onSuccess: (fault) => {
      ...
      queryClient.invalidateQueries({ queryKey: qk.stats.operations })
      showToast({
        tone: fault.status === 'resolved' || fault.status === 'closed' ? 'success' : 'info',
        title: `تم تحديث العطل إلى «${FAULT_STATUS_LABELS[fault.status]}»`,
        description:
          fault.status === 'closed' ? 'أُغلِق العطل وأُعيد تقييم حالة الأصل.' : undefined,
      })
  ...
  return (
    <>
      ...
      <Section>
        <KpiGrid cols={4}>
          ...
          <KpiCard
            label="تم إصلاحها"
            value={formatNumber(
              faults.filter((f) => f.status === 'resolved' || f.status === 'closed').length,
            )}
            icon="quality"
            tone="success"
            loading={faultsQuery.isPending}
      ...
      <DataTable
        ...
        columns={[
          ...
          },
          {
            ...
            header: 'الحل المنفذ',
            render: (f) =>
              f.resolution ?? <span style={{ color: 'var(--text-faint)' }}>لم يسجل بعد</span>,
          },
          {
            key: 'severity',
            header: 'الخطورة',
                ...
                <Button
                  ...
                  onClick={() =>
                    next === 'resolved'
                      ...
                      : advance.mutate({ id: f.id, status: next })
                  }
                >
      ...
      <Modal
        open={resolvingFaultId !== null}
        onClose={() => setResolvingFaultId(null)}
        title="حل العطل"
        subtitle="يتطلب الخادم توثيق السبب الجذري والحل المنفذ"
        footer={
          <>
            <Button variant="ghost" onClick={() => setResolvingFaultId(null)}>
              إلغاء
            </Button>
            <Button
              variant="success"
              loading={advance.isPending}
              disabled={rootCause.trim().length < 3 || resolution.trim().length < 3}
              onClick={() =>
                resolvingFaultId &&
                advance.mutate({
                  id: resolvingFaultId,
      ...
      >
        <Field label="السبب الجذري" required>
          {(props) => (
            <textarea
              {...props}
              rows={3}
        ...
        <Field label="الحل المنفذ" required>
          {(props) => (
            <textarea
              {...props}
              rows={3}
