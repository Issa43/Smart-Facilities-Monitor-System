...
export function WorkOrderDetailPage() {
  ...
  const toggleTask = useMutation({
    ...
    onError: (error) =>
      showToast({
        tone: 'critical',
        title: 'قائمة الفحص غير متاحة',
        description: error instanceof Error ? error.message : undefined,
      }),
  })

  const changeStatus = useMutation({
    mutationFn: ({ status, reason }: { status: WorkOrderStatus; reason?: string }) =>
      setWorkOrderStatus(orderId, status, reason),
    onSuccess: (updated) => {
      invalidate()
      setCloseOpen(false)
      setCancelOpen(false)
    ...
    onError: (error) =>
      showToast({
        tone: 'critical',
        title: 'تعذّر تنفيذ إجراء أمر الصيانة',
        description: error instanceof Error ? error.message : undefined,
      }),
  })
  ...
  const saveNotes = useMutation({
    ...
    onError: (error) =>
      showToast({
        tone: 'critical',
        title: 'ملاحظات التنفيذ غير مدعومة',
        description: error instanceof Error ? error.message : undefined,
      }),
  })
  ...
  const tasksReady = order.tasks.length === 0 || allTasksDone
  const isClosed = order.status === 'closed' || order.status === 'cancelled'
  const canCancel = order.status === 'open' || order.status === 'assigned'
        ...
        actions={
          !isClosed && (
            <>
              ...
              {canCancel && (
                <Button variant="critical" onClick={() => setCancelOpen(true)}>
              ...
              {order.status === 'completed' && (
                <Button variant="success" onClick={() => changeStatus.mutate({ status: 'closed' })}>
                  <Check size={15} strokeWidth={2.2} />
                  إغلاق الأمر المكتمل
                </Button>
              )}
        ...
        <Panel>
          ...
          <DescriptionList
            items={[
              ...
              { label: 'نوع الأصل', value: order.assetType },
              ...
              ...(order.status === 'cancelled'
                ? [
                    ...
                    {
                      label: 'ألغاه',
                      value: userName(order.cancelledById ?? null),
                    },
                  ]
                : []),
              {
      ...
      >
        {!allTasksDone && order.tasks.length > 0 && (
          <Alert
            ...
            description={`${formatNumber(order.tasks.length - doneTasks)} بند لم يُعلَّم كمنجز بعد. يجب استكمال جميع البنود قبل إكمال الأمر.`}
          />
        )}
        ...
        footer={
          <>
            ...
            <Button
              ...
              onClick={() =>
                changeStatus.mutate({ status: 'cancelled', reason: cancellationReason })
              }
            >
              تأكيد الإلغاء
            </Button>
        ...
        <Field label="سبب الإلغاء" required>
          {(props) => (
            <textarea
              ...
              placeholder="اذكر السبب التشغيلي لإلغاء الأمر…"
            />
          )}
        </Field>
      </Modal>
