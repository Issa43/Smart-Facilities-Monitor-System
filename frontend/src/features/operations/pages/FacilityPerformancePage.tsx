...
export function FacilityPerformancePage() {
  ...
  const rows = facilities.map((facility) => {
    ...
    return {
      ...
      health,
      openOrders: facilityOrders.filter((o) =>
        ['open', 'assigned', 'in_progress'].includes(o.status),
      ).length,
      completionRate,
      openFaults: facilityFaults.filter((f) => f.status !== 'closed').length,
    }
  })
  const uptimeRows = rows.flatMap((row) =>
    row.facility.uptimePercent === null
      ? []
      : [{ ...row, uptimePercent: row.facility.uptimePercent }],
  )

  const dataError = [facilitiesQuery, assetsQuery, ordersQuery, faultsQuery, statsQuery].find(
    (query) => query.isError,
  )?.error
          ...
          <KpiCard
            ...
            value={
              stats?.averageUptime === null
                ? 'غير متاح'
                : `${formatDecimal(stats?.averageUptime ?? 0)}%`
            }
            icon="progress"
            tone={
              stats?.averageUptime === null
                ? 'neutral'
                : (stats?.averageUptime ?? 0) >= 95
                  ? 'success'
                  : 'warning'
            }
            loading={statsQuery.isPending}
            progress={stats?.averageUptime ?? undefined}
          />
          <KpiCard
              ...
              <ComparisonBars
                bars={uptimeRows
                  ...
                  .map((row) => ({
                    ...
                    value: row.uptimePercent,
                    max: 100,
                    tone:
                      row.uptimePercent >= 96
                        ? 'success'
                        : row.uptimePercent >= 90
                          ? 'warning'
                          : 'critical',
                    display: `${formatDecimal(row.uptimePercent)}%`,
                  }))}
              />
            )}
          </Panel>
          ...
          <DataTable
            ...
            columns={[
              ...
              },
              {
                ...
                header: 'الجاهزية',
                width: '160px',
                sortValue: (row) => row.facility.uptimePercent ?? -1,
                render: (row) =>
                  row.facility.uptimePercent === null ? (
                    <span className={shared.hint}>غير متاح</span>
                  ) : (
      ...
      <Section>
        <Panel title="الملخص التشغيلي">
          <p className={shared.hint}>
            ...
            ومتوسط صحة أصول <strong>{formatPercent(stats?.assetHealth ?? 0)}</strong>. أُنجز{' '}
