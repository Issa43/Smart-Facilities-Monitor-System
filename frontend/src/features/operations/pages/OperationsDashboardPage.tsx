...
export function OperationsDashboardPage() {
  ...
  const openOrders =
    workOrdersQuery.data
      ?.filter((order) => ['open', 'assigned', 'in_progress'].includes(order.status))
      .sort((a, b) => a.scheduledDate.localeCompare(b.scheduledDate))
      .slice(0, 6) ?? []
  const openFaults = faultsQuery.data?.filter((fault) => fault.status !== 'closed') ?? []
          ...
          <KpiCard
            ...
            footnote={
              stats?.averageUptime === null
                ? 'الجاهزية غير متاحة من API'
                : `متوسط الجاهزية ${formatDecimal(stats?.averageUptime ?? 0)}%`
            }
          />
          <KpiCard
            label="إجمالي الأصول"
      ...
      <Section>
        <div className={shared.chartRow}>
          ...
          >
            ...
            ) : (stats?.maintenanceTrend.length ?? 0) === 0 ? (
              <StateCard bare title="لا توجد أوامر صيانة مُنجزة خلال آخر ستة أشهر" />
            ) : (
              <ColumnChart
            ...
            <div style={{ marginTop: 20 }} className={shared.metricList}>
              ...
              <div className={shared.metricRow}>
                ...
                <span className={shared.metricValue}>
                  {formatNumber(
                    assetsQuery.data?.filter((a) => a.status === 'under_maintenance').length ?? 0,
                  )}
                </span>
              </div>
            </div>
              ...
              <div className={shared.strip}>
                {facilitiesQuery.data?.map((facility) => (
                  ...
                  >
                    ...
                    {facility.uptimePercent === null ? (
                      <span className={shared.hint}>مؤشر الجاهزية غير متاح من الخادم الحالي</span>
                    ) : (
                      <ProgressBar
