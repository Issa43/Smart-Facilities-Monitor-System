...
const FILTERS: { value: StageFilter; label: string }[] = [
  ...
  { value: 'needs_modification', label: 'تحتاج إلى تعديل' },
...
export function StagesPage() {
  ...
  const selectedProjectStartDate = projectsQuery.data
  ...
  const create = useMutation({
    ...
    onError: (error) => mutationError('تعذّر إنشاء المرحلة', error),
  })

  function submitStage(values: StageFormValues) {
    const projectStartDate = projectsQuery.data
      ?.find((project) => project.id === values.projectId)
      ?.startDate.slice(0, 10)
    if (projectStartDate && values.startDate < projectStartDate) {
      setError('startDate', {
        type: 'validate',
        message: 'تاريخ بداية المرحلة لا يمكن أن يسبق تاريخ بداية المشروع',
      })
      return
    }
  ...
  const dataError = [projectsQuery, stagesQuery].find((query) => query.isError)?.error
  ...
  return (
    <>
      ...
      <DataTable
        loading={projectsQuery.isPending || stagesQuery.isPending}
        rows={filtered}
      ...
      <Modal
        ...
        footer={
          <>
            ...
            <Button loading={create.isPending} onClick={handleSubmit(submitStage)}>
              حفظ المرحلة
            </Button>
          </>
        }
      >
        <form onSubmit={handleSubmit(submitStage)} noValidate>
