...
export function ConstructionDashboardPage() {
  ...
  const underReview =
    stagesQuery.data?.filter(
      (s) =>
        s.progressPercent === 100 && s.status === 'in_progress' && myProjectIds.has(s.projectId),
    ) ?? []

  const dataError = [statsQuery, projectsQuery, stagesQuery, materialsQuery, requestsQuery].find(
    (query) => query.isError,
  )?.error
      ...
      <Section>
        <div className={shared.chartRow}>
          ...
          <Panel title="حالة المراحل" subtitle="توزيع مراحل مشاريعك">
            ...
            ) : (stats?.stagesByStatus.length ?? 0) === 0 ? (
              <StateCard bare title="لا توجد مراحل لعرض توزيع حالاتها" />
            ) : (
              <DonutChart
