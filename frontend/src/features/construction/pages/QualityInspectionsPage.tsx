...
export function QualityInspectionsPage() {
  ...
  const [pendingDelete, setPendingDelete] = useState<QualityInspection | null>(null)
  ...
  const create = useMutation({
    ...
    onError: (error) => mutationError('تعذّر تسجيل فحص الجودة', error),
  })
  ...
  const remove = useMutation({
    ...
    onSuccess: () => {
      ...
      showToast({ tone: 'success', title: 'تم حذف فحص الجودة' })
    },
    onError: (error) => mutationError('تعذّر حذف فحص الجودة', error),
  })
  ...
  const dataError = [projectsQuery, stagesQuery, inspectionsQuery].find(
    (query) => query.isError,
  )?.error
  ...
  return (
    <>
      ...
      <Section>
        <KpiGrid cols={3}>
          <KpiCard
            ...
            icon="quality"
            tone="primary"
            loading={projectsQuery.isPending || stagesQuery.isPending || inspectionsQuery.isPending}
          />
          <KpiCard
            label="متوسط الدرجة"
      ...
      {projectsQuery.isPending || stagesQuery.isPending || inspectionsQuery.isPending ? (
        <Panel>
          <ProgressBar value={0} />
        </Panel>
      ) : filtered.length === 0 ? (
          ...
          {filtered.map((inspection) => (
            <article key={inspection.id} className={styles.card}>
              ...
              <div className={styles.cardFoot}>
                ...
                <Button size="sm" variant="critical" onClick={() => setPendingDelete(inspection)}>
                  <Trash2 size={12} /> حذف
                </Button>
              </div>
            </article>
      ...
      >
        سيتم حذف نتيجة الفحص من سجل الجودة.
      </Modal>
    </>
  )
}
