...
export function MaterialRequestsPage() {
  ...
  const updateStatus = useMutation({
    ...
    onError: (error) => mutationError('تعذّر تحديث طلب المواد', error),
  })
  ...
  const dataError = [projectsQuery, requestsQuery].find((query) => query.isError)?.error
      ...
      <Section>
        <KpiGrid cols={4}>
          <KpiCard
            ...
            tone="primary"
            loading={projectsQuery.isPending || requestsQuery.isPending}
          />
          <KpiCard
            label="قيد المعالجة"
