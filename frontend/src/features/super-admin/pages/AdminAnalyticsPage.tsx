...
export function AdminAnalyticsPage() {
  ...
  const facilitiesWithUptime = facilities.flatMap((facility) =>
    facility.uptimePercent === null ? [] : [{ ...facility, uptimePercent: facility.uptimePercent }],
  )
  ...
  const pageError = statsQuery.error ?? projectsQuery.error ?? facilitiesQuery.error
  if (pageError) return <ErrorState error={pageError} />
    ...
    <>
      ...
      <Section>
        <KpiGrid>
          ...
          <KpiCard
            label="مشاريع متأخرة"
            value={formatNumber(stats?.delayedProjects ?? 0)}
            icon="stages"
            tone={(stats?.delayedProjects ?? 0) > 0 ? 'warning' : 'success'}
            loading={statsQuery.isPending}
            footnote={
              (stats?.delayedProjects ?? 0) > 0
                ? 'مشاريع نشطة تجاوزت تاريخ الانتهاء المتوقع'
                : 'جميع المشاريع النشطة ضمن الجدول'
            }
            footnoteTone={(stats?.delayedProjects ?? 0) > 0 ? 'warning' : 'success'}
          />
          <KpiCard
            label="صحة الأصول"
      ...
      <Section>
        <Panel
          title="اتجاه تحديثات الإنجاز — آخر ستة أشهر"
          subtitle="متوسط نسب التقدم الفعلية المسجلة في سجل المراحل لكل شهر"
        >
          {statsQuery.isPending ? (
            <SkeletonLines count={5} />
          ) : (stats?.progressTrend.length ?? 0) === 0 ? (
            <StateCard bare title="لا توجد تحديثات تقدم مسجلة خلال آخر ستة أشهر" />
          ) : (
            <AreaChart
        ...
        <SplitGrid even>
          <Panel title="مزيج المشاريع" subtitle="توزيع المشاريع حسب نوع المنشأة">
            {projectsQuery.isPending ? (
              ...
              <StateCard bare title="لا توجد مشاريع لعرض توزيعها" />
            ) : (
              <DonutChart
                slices={byType}
          ...
          <Panel title="حالة الأصول التشغيلية" subtitle="توزيع الأصول حسب حالتها الحالية">
            {statsQuery.isPending ? (
              <SkeletonLines count={4} />
            ) : (stats?.assetsByStatus.length ?? 0) === 0 ? (
              <StateCard bare title="لا توجد أصول لعرض توزيع حالاتها" />
            ) : (
              <DonutChart
        ...
        <SplitGrid>
          <Panel title="مقارنة أداء المنشآت" subtitle="نسبة الجاهزية التشغيلية (Uptime) لكل منشأة">
            {facilitiesQuery.isPending ? (
              <SkeletonLines count={5} />
            ) : facilitiesWithUptime.length === 0 ? (
              <StateCard bare title="مؤشر الجاهزية غير متاح من الخادم الحالي" />
            ) : (
              <ComparisonBars
                bars={facilitiesWithUptime
                  .slice()
          ...
          <Panel title="الملخص التنفيذي">
            <p className={shared.hint}>
              ...
              <strong>{formatNumber(stats?.totalProjects ?? 0)}</strong> مشروعاً مسجلاً، مع{' '}
              <strong>{formatNumber(stats?.delayedProjects ?? 0)}</strong> مشروعاً نشطاً تجاوز تاريخ
              الانتهاء المتوقع ويتطلب مراجعة الجدول الزمني.
            </p>
