...
import { presentAuditEntry, selectLatestOperations } from '../auditPresentation'
...
export function AdminDashboardPage() {
  ...
  const latestOperations = selectLatestOperations(auditQuery.data ?? [])
    ...
    <>
      ...
      <Section>
        <div className={shared.chartRow}>
          <Panel
            title="اتجاه تحديثات الإنجاز"
            subtitle="متوسط نسب التقدم الفعلية المسجلة في سجل المراحل خلال آخر ستة أشهر"
          >
          ...
          <Panel title="توزيع المشاريع" subtitle="حسب الحالة الحالية">
            ...
            ) : (stats?.projectsByStatus.length ?? 0) === 0 ? (
        ...
        <div className={shared.chartRow}>
          <Panel title="آخر العمليات في النظام" subtitle="سجل التدقيق — أحدث 8 عمليات">
            {auditQuery.isPending ? (
              ...
              <ErrorState bare error={auditQuery.error} onRetry={() => auditQuery.refetch()} />
            ) : latestOperations.length === 0 ? (
              <StateCard bare title="لا توجد عمليات مسجلة حتى الآن" />
            ) : (
              <Timeline
                entries={latestOperations.map((entry) => {
                  const presentation = presentAuditEntry(entry)
                  return {
                    id: entry.id,
                    tone: presentation.tone,
                    title: presentation.title,
                    body: `${presentation.entityLabel} · ${presentation.reference}`,
                    meta: `${entry.actorName ?? 'مستخدم محذوف'} · ${formatRelative(entry.createdAt)}`,
                  }
                })}
              />
            )}
