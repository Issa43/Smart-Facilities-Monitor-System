...
import { LocationPicker, type LatLng } from '@/components/ui/LocationPicker/LocationPicker'
...
const schema = z
  .object({
    ...
    // Absent until a point is picked. Never derived from the textual
    // location, and never defaulted to the map's preview centre.
    latitude: z
      .number()
  ...
  // A half-picked point is never valid: both or neither.
  .refine((values) => (values.latitude === null) === (values.longitude === null), {
    message: 'حدّد نقطة كاملة على الخريطة',
    path: ['latitude'],
  })
...
export function ProjectFormPage({ mode }: { mode: 'create' | 'edit' }) {
  ...
  function handlePick(point: LatLng) {
    setValue('latitude', point.latitude, { shouldDirty: true, shouldValidate: true })
    setValue('longitude', point.longitude, { shouldDirty: true, shouldValidate: true })
  }
        ...
        <form onSubmit={handleSubmit((values) => save.mutate(values))} noValidate>
          <Section>
            <Panel title="البيانات الأساسية">
              ...
              {/* The textual location above names the site; the map below fixes
                  its exact point. The two are independent: nothing here turns
                  the text into coordinates. */}
              <LocationPicker
                label="الموقع الجغرافي للمشروع"
                hint="اضغط على الخريطة لتحديد الموقع الدقيق للمشروع."
                value={picked}
                onChange={handlePick}
              />
              {errors.latitude?.message && (
                <Alert tone="critical" title={errors.latitude.message} />
              )}
            ...
            <Panel title="الجدول الزمني والإسناد">
              ...
              <FieldRow>
                <Field
                  ...
                  hint="تتغير الحالة فقط عبر إجراءات دورة حياة المشروع المعتمدة"
                  required
                >
                  {(props) => (
                    <select {...props} {...register('status')} disabled>
          ...
          <Section>
            <Panel title="صورة المشروع" subtitle="اختيارية — تُعرض في بطاقة المشروع">
              <UploadZone
                ...
                onFilesChange={(files) => setProjectImage(files[0] ?? null)}
              />
            </Panel>
          </Section>
