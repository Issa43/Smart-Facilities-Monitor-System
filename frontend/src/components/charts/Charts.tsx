...
function ChartTooltip({
  ...
  return (
    <div className={styles.tooltip} dir="rtl" role="status">
...
export function ChartLegend({ entries }: { entries: LegendEntry[] }) {
  return (
    <ul className={styles.legend} aria-label="مفتاح المخطط">
      {entries.map((entry) => (
...
export function AreaChart<T extends object>({
  ...
  return (
    <div
      ...
      role="img"
      aria-label={ariaLabel ?? `مخطط اتجاه ${series.map((entry) => entry.label).join(' و')}`}
    >
      <ResponsiveContainer width="100%" height={height}>
        <RechartsAreaChart data={data} margin={CHART_MARGIN} accessibilityLayer>
          <defs>
          ...
          <YAxis {...Y_AXIS_PROPS} domain={suffix === '%' ? [0, 100] : undefined} />
          <Tooltip
...
export function ColumnChart<T extends object>({
  ...
  return (
    <div className={styles.wrap} role="img" aria-label={ariaLabel ?? `مخطط ${label}`}>
      <ResponsiveContainer width="100%" height={height}>
        <RechartsBarChart data={data} margin={CHART_MARGIN} barCategoryGap="28%" accessibilityLayer>
          <CartesianGrid {...GRID_PROPS} />
          <XAxis dataKey={xKey as never} {...X_AXIS_PROPS} />
...
export function DonutChart({
  ...
  return (
    <div
      className={styles.wrap}
      role="img"
      aria-label={ariaLabel ?? `مخطط توزيع ${centerLabel ?? 'البيانات'}`}
    >
...
export function ComparisonBars({ bars }: { bars: ComparisonBar[] }) {
  ...
  return (
    <div className={styles.bars} role="img" aria-label="مخطط مقارنة">
      {bars.map((bar, index) => {
