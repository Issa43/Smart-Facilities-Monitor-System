...
// Local-development only. Wrapping the dynamic import in the DEV check lets
// the bundler drop the page entirely from a production build, so no chunk for
// it is emitted at all.
const DevPasswordResetPage = import.meta.env.DEV
  ? lazy(() =>
      import('@/features/auth/pages/DevPasswordResetPage').then((m) => ({
        default: m.DevPasswordResetPage,
      })),
    )
  : null
...
const AdminMaterialRequestsPage = lazy(() =>
  import('@/features/super-admin/pages/AdminMaterialRequestsPage').then((m) => ({
    default: m.AdminMaterialRequestsPage,
  })),
)
...
const AdminAiCameraConfigurationPage = lazy(() =>
  import('@/features/super-admin/pages/AdminAiCameraConfigurationPage').then((m) => ({
    default: m.AdminAiCameraConfigurationPage,
  })),
)
...
const ConstructionReportsPage = lazy(() =>
  import('@/features/construction/pages/ConstructionReportsPage').then((m) => ({
    default: m.ConstructionReportsPage,
  })),
)
...
const CameraEventsPage = lazy(() =>
  import('@/features/security/pages/CameraEventsPage').then((m) => ({
    default: m.CameraEventsPage,
  })),
)
const CameraEventDetailPage = lazy(() =>
  import('@/features/security/pages/CameraEventDetailPage').then((m) => ({
    default: m.CameraEventDetailPage,
  })),
)
...
const SafetyAlertsPage = lazy(() =>
  import('@/features/safety/pages/SafetyAlertsPage').then((m) => ({ default: m.SafetyAlertsPage })),
)
const SafetyAlertDetailPage = lazy(() =>
  import('@/features/safety/pages/SafetyAlertDetailPage').then((m) => ({
    default: m.SafetyAlertDetailPage,
  })),
)
  ...
  return (
    ...
    <Suspense fallback={<RouteFallback />}>
      <Routes>
        ...
        {/* Local-development shortcut for obtaining a reset link. Absent from
            production builds, and its backend endpoint refuses to run unless
            the server has DEBUG on. */}
        {import.meta.env.DEV && DevPasswordResetPage && (
          <Route path="/dev/password-reset" element={<DevPasswordResetPage />} />
        )}
        ...
        <Route element={<ProtectedRoute role="super_admin" />}>
          ...
          <Route path="/admin/material-requests" element={<AdminMaterialRequestsPage />} />
          ...
          <Route
            path="/admin/ai-camera-configuration"
            element={<AdminAiCameraConfigurationPage />}
          />
        </Route>
        ...
        <Route element={<ProtectedRoute role="construction_manager" />}>
          ...
          <Route path="/construction/reports" element={<ConstructionReportsPage />} />
          <Route path="/construction/documents" element={<DocumentsPage />} />
          <Route path="/construction/photos" element={<SitePhotosPage />} />
          <Route
            path="/construction/safety-alerts"
            element={<SafetyAlertsPage basePath="/construction" />}
          />
        ...
        <Route element={<ProtectedRoute role="operations_manager" />}>
          ...
          />
          <Route path="/operations/safety-alerts/:alertId" element={<SafetyAlertDetailPage />} />
        </Route>
        ...
        <Route element={<ProtectedRoute role="security_officer" />}>
          ...
          <Route path="/security/events" element={<CameraEventsPage />} />
          <Route path="/security/events/:eventId" element={<CameraEventDetailPage />} />
